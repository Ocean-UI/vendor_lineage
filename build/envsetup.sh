...
function oceanremote()
{
    ...
    local OCEAN_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$OCEAN_USER" ]
    then
        git remote add ocean ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add ocean ssh://$LINEAGE_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
...
function lineagerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Ocean-UI Gerrit Rebase Usage: "
